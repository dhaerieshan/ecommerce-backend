...
@Test
  ...
  @Test
    ...
    List<Product> result = productRepository.findTop5ByCategoryAndIdNot(subCategory, product.getId());
...
@Test
  ...
  void testFindByCategoryIds() {
    ...
    List<Product>result = productRepository.findByCategoryIds(List.of(parentCategory.getId(), subCategory.getId()));
    assertNotNull(result);
    assertEquals(1, result.size());
...
void testFindByNameContainingIgnoreCaseOrDescriptionContainingIgnoreCaseOrCategory_NameContainingIgnoreCase() {
  Category category = new Category();
  category.setName("Electronics");
  category = categoryRepository.save(category);
